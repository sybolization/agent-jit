...
import { enumBaseOf, schemaViewText, type SchemaView } from "../tools/schemaView.js";
...
export function buildToolNode(
  ...
  for (const arg of statement.args) {
    ...
    if (error) {
      const expectedText =
        parameter.kind === "enum" && parameter.legalValues
...
export function validateMapBindings(
  ...
  for (const [param, field] of Object.entries(node.bindings)) {
    ...
    if (!fieldCompatibleWithParam(prop, paramSpec)) {
      const expectedText =
        paramSpec.kind === "enum" && paramSpec.legalValues
 ...
 */
function fieldCompatibleWithParam(view: SchemaView, spec: ToolParamSpec): boolean {
  ...
  const matches = (candidate: SchemaView): boolean => {
    if (candidate.kind === "enum") {
      ...
      if (base === "string") return kind === "string" || kind === "enum";
      if (base === "number") return kind === "int" || kind === "number" || kind === "enum";
      return kind === "enum";
    }
    if (kind === "enum") {
      ...
      if (candidate.kind === "string") return hasString;
      if (numeric.has(candidate.kind)) return hasNumber;
      return false;
    }
