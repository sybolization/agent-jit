...
function buildDslSystemPrompt(task: R4eTask): string {
  return [
    ...
    "- <callee>：已注册工具 id，或语言关键字 map / take / filter / sort / compute / select / join / return",
    "- <value>：字符串（双引号）、数字、布尔、null，或先前定义的变量名（裸标识符即引用，定义数据流边）",
    "- take：第一个位置参数是源数组，第二个位置参数是截取条数",
    "- filter：第一个位置参数是源数组，其余参数是等值条件（<字段>=<字面量>）",
    ...
    "4. 分支要互补：ratio > 0.15 与 ratio <= 0.15 各写一次 select",
    "5. 编译失败时，根据返回的诊断修正 DSL，再次调用 submit_program 重新提交，直到成功为止",
  ].join("\n");
}
