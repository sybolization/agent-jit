 ...
 * - exposed_bytes：喂给模型的中间数据量（每轮 toolResult JSON 字节累计）；
 * - usage / llm_ms / tool_ms / e2e_ms。
 */
...
export async function runIterativeToolCalling(options: IterativeOptions): Promise<IterativeToolResult> {
  ...
  const minConsecutiveNoTool = options.minConsecutiveNoTool ?? 2;
  ...
  for (let step = 0; step < maxSteps; step += 1) {
    ...
    for (const call of toolCalls) {
      ...
      messages.push({ role: "toolResult", toolCallId: call.id, toolName: call.name, content: resultText, isError });
    }
  }
