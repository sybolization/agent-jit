...
| **E2 语言实验 R2：positional vs named 2×2 factorial** | 只测调用形态（named/positional × zero/few-shot），IR/runtime 不变（**已落地** 2026-08-07：A=0% / B=100% / C=50% / D=100%，H5 部分获支持） | parser 中性支持位置参数（`ParsedArg.key` 可选）；compiler 加 `allowPositionalArgs` 开关 + `POSITIONAL_ARG_NOT_ALLOWED` 探针；新增 `src/experiments/taskSpec.ts`（task_correctness 与执行解耦） | 2×2 factorial 四臂 × 10 样本；四层 metric（parse/conformance/execution/task）+ 2×2 效应 | 结论：positional 更贴模型先验（同 zero-shot C 50% vs A 0%），few-shot 仍最大杠杆（+1.50/+1.60）；**反馈循环 0 次成功挽回，成功全为 first-attempt**；语言层建议 positional+few-shot 组合 | 详见 `experiment_result/语言实验-第二轮结果.md`；A 臂 31 次 `POSITIONAL_ARG_NOT_ALLOWED` = named 语法与模型先验冲突的可量化证据（探针方法论）；修复 parser 死循环（`return top` 无括号形态） |
