...
| **E1 语言实验 R1：callable reference 三臂** | 只测 tool 参数表达（quoted-zs / quoted-fs / symbolic-zs），IR/runtime 不变（**已落地** 2026-08-07：A=0% / B=100% / C=40%，H4 未获支持） | compiler 加 `allowCallableRef` 开关 + `EXPECTED_STRING_GOT_CALLABLE_REF` 专用诊断码；harness 升级多臂批量 + 统计 | 每臂 10 样本；friction 指标 = error 分布计数 | 结论：真正摩擦是位置参数/无括号 return/参数名幻觉，非裸标识符；few-shot 完胜 | 详见 `docs/语言实验-第一轮结果.md`；第二轮候选：位置参数 vs 命名参数 |
