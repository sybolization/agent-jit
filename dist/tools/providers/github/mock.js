...
export const ADVERSARIAL_REPOS = [
 ...
 */
export function createAdversarialGithubTools() {
    const contracts = [
        defineTool({
            ...
            inputSchema: Type.Object({ query: Type.String(), limit: Type.Optional(Type.Integer()) }, { additionalProperties: false }),
        ...
        defineTool({
            ...
            outputSchema: Type.Object({ full_name: Type.String(), forks: Type.Integer(), stars: Type.Integer(), language: Type.String() }, { additionalProperties: false }),
        }),
        defineTool({
        ...
        defineTool({
            ...
            inputSchema: Type.Object({ full_name: Type.String(), per_page: Type.Optional(Type.Integer()) }, { additionalProperties: false }),
            outputSchema: Type.Object({ full_name: Type.String(), score: Type.Integer() }, { additionalProperties: false }),
        }),
    ];
    const contractOf = (id) => {
        ...
        if (!contract)
            throw new Error(`mock: 未注册的工具 ${id}`);
        return contract;
    };
    ...
    return [
        {
            ...
            execute: async (args) => {
                const limit = Number(args.limit ?? ADVERSARIAL_REPOS.length);
 ...
 * 形态（仅 manifest 臂可见）。
 */
export function createOpaqueAdversarialGithubTools() {
    const contracts = [
        defineTool({
            id: "github.search_repositories",
            label: "Search GitHub repositories",
        ...
        defineTool({
            id: "github.get_contributor_stats",
            label: "Get repository contributor stats",
            description: "获取仓库贡献者统计，返回该仓库贡献者路径的统一可比较数值（与提交路径数值同尺度，可直接排序比较）。",
            inputSchema: Type.Object({ full_name: Type.String() }, { additionalProperties: false }),
            outputSchema: Type.Object({
                repo_ref: Type.String({ description: "repository identity" }),
        ...
        defineTool({
            id: "github.list_commits",
            label: "List repository commits",
            description: "获取仓库提交统计，返回该仓库提交路径的统一可比较数值（与贡献者路径数值同尺度，可直接排序比较）。",
            inputSchema: Type.Object({ full_name: Type.String(), per_page: Type.Optional(Type.Integer()) }, { additionalProperties: false }),
            outputSchema: Type.Object({
                repo_ref: Type.String({ description: "repository identity" }),
                aggregate_value: Type.Integer({ description: "score" }),
...
export function createMockGithubTools(options = {}) {
    const [minDelay = 20, maxDelay = 100] = options.delayMs ?? [20, 100];
    const count = options.repositoryCount ?? 10;
    const delay = () => new Promise((resolve) => setTimeout(resolve, minDelay + Math.random() * (maxDelay - minDelay)));
